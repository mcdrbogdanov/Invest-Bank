...
async def top_up_handler(message: Message):
    ...
    await message.answer(f"Отправьте любую сумму на QIWI +79532814741 с комментарием {message.from_id}. Если Вы забудете написать комментарий {message.from_id}, то Вам не будут зачислены средства.\nКогда транзакция пройдёт успешно, через некоторое время Вам придут средства на баланс бота.")
